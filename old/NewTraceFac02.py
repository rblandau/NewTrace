    ...
    def tracef(self, level, facility, line):
        ...
        if level <= self.tracelevel:
            ...
            if self.traceme:
                # Get a timestamp
                self.vecT = localtime()
                (self.yr,self.mo,self.da,self.hr,self.min,self.sec,self.x,self.y,self.z) \
                    = self.vecT
                self.ascT = "%4d%02d%02d_%02d%02d%02d" % (self.yr,self.mo,self.da,self.hr,self.min,self.sec)
#                self.linestart = self.ascT + " " + "%1d"%level + " " + facility + " " 
